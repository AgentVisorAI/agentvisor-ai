...
Recent-CVE reachability triage and remediations for the AgentBridge workspace.
...
- `crates/ab-sandbox/src/wasm_policy.rs::tests::invalid_wasm_rejected_at_load`
- `crates/ab-sandbox/src/wasm_policy.rs::tests::missing_exports_fail_closed`
- `crates/ab-sandbox/src/wasm_policy.rs::tests::hostile_infinite_loop_fails_closed_via_fuel_and_epoch`
- `crates/ab-sandbox/src/wasm_policy.rs::tests::memory_bomb_policy_fails_closed_via_store_limits` (new)
- `crates/ab-sandbox/src/wasm_policy.rs::tests::hostile_return_codes_all_deny` (new)
...
`crates/ab-loopdetect/src/onnx_embed.rs:30`, which invokes the ONNX parser
...
- **rustls-webpki 0.102.8 CVEs (2026-0049, 0098, 0099, 0104)** — reach only via
  ...
  control the NATS server the operator configures. The AgentBridge threat
...
Embedded broker (crates/ab-bridge/tests/embedded_contract.rs):
...
Cold-store MAC (crates/ab-bridge/src/cold_store.rs, `cold-store` feature):
...
Wasm sandbox (crates/ab-sandbox/src/wasm_policy.rs):
...
- Sandbox tests: `cargo test -p ab-sandbox` — 29 pass (2 new adversarial).
