...
//! `crates/av-state/tests/e2e_race_velocity.rs`. The type predates the
//! current admission / loop-breaker plumbing and was never connected to
//! either the breaker's `min_tokens` gate (in `av_loopdetect`) or any
...
//! velocity in the breaker, extend `av_loopdetect::Breaker` with its own
...
impl TokenVelocity {
    ...
    pub fn record(&self, tokens: u64) -> u64 {
        self.record_at(av_core::time::now_ms(), tokens)
    }
