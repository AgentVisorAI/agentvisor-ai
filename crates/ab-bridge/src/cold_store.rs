...
impl ColdArchive {
    ...
    fn pending_path(&self, topic: &str, event_uid: &str) -> std::path::PathBuf {
        self.pending_dir.join(format!(
            "{}.json",
            ab_core::digest::sha256_hex(format!("{topic}:{event_uid}").as_bytes())
        ))
    }
}
...
fn pending_mac(control_key: &[u8; 32], pending: &PendingColdEvent) -> Result<String, BusError> {
    ...
    let canonical =
        ab_receipts::canonicalize(&value).map_err(|error| BusError::Backend(error.to_string()))?;
    let mut mac =
...
fn remove_pending(path: &std::path::Path) -> Result<(), BusError> {
    ...
    match std::fs::remove_file(path) {
        Ok(()) => ab_core::fsutil::sync_directory(parent).map_err(BusError::Io),
