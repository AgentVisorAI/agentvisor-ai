...
impl SessionSummary {
    pub(crate) fn from_session(session: &crate::session::Session) -> Self {
        ...
        let stop = av_events::StopReason::from_id(stop_id);
        Self {
            id: session.id.clone(),
...
pub async fn list_sessions(
    ...
    let response = no_store_json_response(json!({
        "sessions": summaries,
        "generated_at_ms": av_core::time::now_ms(),
...
mod tests {
    ...
    async fn dashboard_index_served_as_html() {
        ...
        assert!(text.contains("AgentVisor AI"), "index should mention product");
    }
    ...
    async fn detail_endpoint_returns_receipt_when_present() {
        ...
        use av_receipts::{CostSummary, Receipt, ReceiptBody, ReceiptSubject, Signer as _, ToolCallSummary};
    ...
    async fn dashboard_endpoints_register_prometheus_metrics() {
        ...
        for line in [
            "av_dashboard_requests_total{endpoint=\"stats\",status=\"ok\"} 1",
            "av_dashboard_requests_total{endpoint=\"list\",status=\"ok\"} 1",
            "av_dashboard_requests_total{endpoint=\"detail\",status=\"ok\"} 1",
            "av_dashboard_requests_total{endpoint=\"detail\",status=\"not_found\"} 1",
            "av_dashboard_request_duration_seconds_count{endpoint=\"stats\"} 1",
        ] {
            assert!(
