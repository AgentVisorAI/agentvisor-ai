...
use av_sandbox::{PolicyDecision, PolicyEngine, WasmPolicy};
use av_state::{BudgetSpec, InMemoryStore, Spend, StateStore};
use serde_json::json;
...
// consume budget forever). The AgentVisor AI Lua script uses `EXPIRE key
...
// `publish/fetch` treat topic as an opaque string but AgentVisor AI
...
// AgentVisor AI stipulates per-agent ordering (D13.14). The embedded broker
...
// prefixes with `b"agentvisor-cold-outbox-v1\0"` before HMAC-ing; a MAC
...
fn cold_store_control_key_derivation_isolates_deployments() {
    let a = av_receipts::Ed25519Signer::from_seed(&[1; 32]);
    let b = av_receipts::Ed25519Signer::from_seed(&[2; 32]);
    let ka = av_harness::control_key_from_signer(&a);
    let kb = av_harness::control_key_from_signer(&b);
    assert_ne!(ka, kb, "signer keys collided — deployments share MAC secret");
    // And the derivation is deterministic per-signer.
    let ka2 = av_harness::control_key_from_signer(&a);
    assert_eq!(ka, ka2);
}
...
fn budget_giant_amount_is_refused_no_silent_wrap() {
    ...
    let budget = av_state::ActionBudget::new(&store, "sess", &spec);
    ...
    match outcome {
        Ok(av_state::BudgetDecision::Refused { .. }) | Err(_) => {}
        Ok(av_state::BudgetDecision::Allowed { .. }) => {
