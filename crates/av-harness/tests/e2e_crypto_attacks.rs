...
// ---------------------------------------------------------------------------
// 9. Genesis concatenation: SHA-256("av-genesis" ‖ session_id) has no
...
fn control_key_derivation_is_deterministic_and_signer_separated() {
    ...
    let k_a1 = av_harness::control_key_from_signer(&a1);
    let k_a2 = av_harness::control_key_from_signer(&a2);
    let k_b = av_harness::control_key_from_signer(&b);
...
fn ed25519_signatures_are_deterministic_no_nonce_reuse_surface() {
    let s = signer();
    let msg = b"agentvisor-crypto-canary";
