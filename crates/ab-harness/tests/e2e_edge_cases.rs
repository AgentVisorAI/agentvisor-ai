...
use ab_events::{AgentIdentity, CharterFile, EventClass};
...
use ab_state::{ActionBudget, BudgetDecision, BudgetSpec, InMemoryStore};
...
async fn session_id_exact_length_boundary() {
    ...
    for good_len in [1usize, 32, 127, 128] {
        ...
        h.insert("x-ab-session", HeaderValue::from_str(&id).unwrap());
        assert!(
...
async fn session_id_visible_ascii_boundary() {
    ...
    for good in ["!", "~", "!~a", "abc-DEF_123.xyz"] {
        let mut h = HeaderMap::new();
        h.insert("x-ab-session", HeaderValue::from_str(good).unwrap());
        assert!(
            state.prepare_chat(&h, chat_payload()).is_ok(),
    ...
    for bad in [
        ...
        assert!(
            ab_core::SessionId::parse(bad).is_err(),
            "SessionId must reject {bad:?}"
        );
    }
    for edge in ["!", "~", "!~a"] {
        assert!(
            ab_core::SessionId::parse(edge).is_ok(),
...
async fn concurrent_close_session_is_idempotent() {
    ...
    let state = state(cfg);
    let mut h = HeaderMap::new();
    h.insert("x-ab-session", HeaderValue::from_static("idem-close"));
    ...
    for h in handles {
        match h.await.unwrap().unwrap() {
            ab_harness::reconciler::FinalizeOutcome::AlreadyClosed => already_count += 1,
