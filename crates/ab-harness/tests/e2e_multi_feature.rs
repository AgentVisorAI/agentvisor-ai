...
use ab_harness::pipeline::PipelineError;
...
async fn session_binding_refuses_workflow_change_after_open() {
    ...
    unsigned.insert("x-ab-workflow", HeaderValue::from_static("unsigned"));
...
async fn tool_intercept_after_session_close_refuses() {
    ...
    state
        .finalizer
        .close_session(session.clone(), ab_events::StopReason::SessionClosed)
        .await
        .unwrap();
