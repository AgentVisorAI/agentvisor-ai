...
use ab_receipts::{canonicalize, Ed25519Signer, EventChain, Keyring, Receipt, Signer};
use ab_sandbox::{PolicyDecision, PolicyEngine, Sandbox, SandboxConfig, ToolVerdict};
...
// each see remaining budget and each spend, blowing the cap. AgentBridge's
...
// drain a per-tool ceiling that was scoped per-session. AgentBridge's
...
// referential verification. AgentBridge's defense: sandbox validates
...
fn receipt_shaped_tool_argument_is_treated_as_opaque_data() {
    ...
    let honest_body = ab_receipts::ReceiptBody {
        ...
        subject: ab_receipts::ReceiptSubject::EventChain {
            chain_head: "aa".repeat(32),
            event_count: 999,
        },
        tool_calls: ab_receipts::ToolCallSummary::default(),
        cost: ab_receipts::CostSummary::default(),
        stop_reason_id: 1,
...
//
// AgentBridge's defense: the sandbox parses ONCE and hands the parsed value
...
fn signature_oracle_reveals_nothing_that_would_forge_a_new_receipt() {
    ...
    for i in 0..200_u64 {
        let body = ab_receipts::ReceiptBody {
            receipt_version: 1,
