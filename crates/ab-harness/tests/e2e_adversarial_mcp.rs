...
async fn tampered_atif_trajectory_is_rejected_on_promotion() {
    ...
    headers.insert("x-ab-workflow", HeaderValue::from_static("unsigned"));
    state
        .prepare_chat(
            &headers,
...
async fn session_hijack_with_different_identity_is_refused() {
    ...
    hostile.insert("x-ab-instance-uid", HeaderValue::from_static("attacker-1"));
    hostile.insert("x-ab-agent-version", HeaderValue::from_static("evil"));
