...
pub fn key_from_signer(signer: &dyn ab_receipts::Signer) -> [u8; 32] {
    let signature = signer.sign(b"agentbridge-active-journal-key-v1");
    let mut key = [0u8; 32];
...
fn build_mac(
    ...
    let canonical = ab_receipts::canonicalize(payload).map_err(|error| error.to_string())?;
    let mut mac = HmacSha256::new_from_slice(key).map_err(|error| error.to_string())?;
    mac.update(b"agentbridge-journal-v1\0");
