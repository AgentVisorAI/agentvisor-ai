...
#[cfg(feature = "onnx")]
use av_loopdetect::OnnxEmbedder;
use av_loopdetect::{BreakerConfig, BreakerVerdict, HashEmbedder, SessionLoopState};
...
fn production_onnx_model_meets_loop_sla() {
    let Ok(model) = std::env::var("AV_ONNX_MODEL_PATH") else {
        eprintln!("SKIPPED (AV_ONNX_MODEL_PATH unset): ONNX loop SLA requires a model");
        return;
    };
