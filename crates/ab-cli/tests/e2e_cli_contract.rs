//! Deep contract checks for the `abctl` operator UI.
//!
...
const BIN: &str = env!("CARGO_BIN_EXE_abctl");

fn run(args: &[&str]) -> Output {
    Command::new(BIN).args(args).output().expect("spawn abctl")
}
...
fn help_mentions_every_subcommand() {
    ...
    let out = run(&["--help"]);
    assert!(out.status.success(), "abctl --help must succeed");
    let text = stdout(&out) + &stderr(&out);
    for cmd in expected {
        assert!(
            text.contains(cmd),
            "abctl --help must document {cmd}, got:\n{text}"
        );
    }
}
...
fn each_subcommand_has_its_own_help() {
    for cmd in [
        ...
        assert!(
            out.status.success(),
            "abctl {cmd} --help must exit 0, got status {:?}\nstderr: {}",
            out.status,
            stderr(&out)
        );
        let text = stdout(&out) + &stderr(&out);
        assert!(
            text.contains(cmd),
            "abctl {cmd} --help output must mention the command name"
        );
    }
}
...
fn version_flag_reports_a_nonempty_version() {
    let out = run(&["--version"]);
    assert!(out.status.success(), "abctl --version must succeed");
    let text = stdout(&out);
    // clap's default: "abctl <VERSION>\n"
    let head = text.trim();
    assert!(
        head.starts_with("abctl "),
        "unexpected --version output: {head:?}"
    );
    let ver = head.trim_start_matches("abctl ").trim();
...
fn cli_exit_status_is_never_a_signal() {
    ...
    for args in [
        ...
        assert!(
            ...
            "abctl {args:?} did not exit cleanly (signal?): status={:?}",
            out.status
        );
    }
}
...
fn help_output_does_not_leak_backtraces_or_debug_types() {
    ...
    for hostile in [
        ...
        assert!(
            ...
            "abctl --help leaks {hostile:?} into user-facing output:\n{all}"
        );
    }
}
...
#[test]
fn abctl_binary_exists_and_is_executable() {
    let path = Path::new(BIN);
    assert!(path.exists(), "abctl binary not found at {BIN}");
    let meta = std::fs::metadata(path).unwrap();
    assert!(meta.is_file(), "abctl at {BIN} is not a regular file");
    ...
    #[cfg(unix)]
    {
        ...
        assert!(
            mode & 0o100 != 0,
            "abctl at {BIN} is not executable: mode={mode:o}"
        );
    }
}
