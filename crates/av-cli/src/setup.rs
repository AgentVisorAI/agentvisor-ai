...
pub struct WizardOutcome {
    ...
    #[allow(dead_code)]
    pub console_config_path: Option<PathBuf>,
    pub start_now: bool,
}

struct ConsoleSetupPlan {
    url: String,
    deployment: String,
    token_file: String,
    bridge_dir: String,
...
fn console_config_path_from(home: &Path) -> PathBuf {
    home.join(".agentvisor").join("console.toml")
}

fn render_console_config(plan: &ConsoleSetupPlan) -> String {
    format!(
        "# Hosted console ingest settings generated by `avctl setup`.\n\
         # Kept outside agentvisor.toml because the daemon rejects unknown top-level tables.\n\
         [console]\n\
         url = \"{}\"\n\
         deployment = \"{}\"\n\
         token_file = \"{}\"\n\
         bridge_dir = \"{}\"\n",
        plan.url, plan.deployment, plan.token_file, plan.bridge_dir
    )
}
...
fn ask_validated_default(
    ...
    for _ in 0..5 {
        ...
        match error_for(answer) {
            None => return Ok(answer.to_owned()),
            Some(reason) => println!("{reason}"),
        }
    }
    anyhow::bail!("no valid answer given");
...
fn console_deployment_error(value: &str) -> Option<String> {
    if value.is_empty() {
        return Some("Deployment id cannot be empty.".to_owned());
    }
    if value.len() > 128 {
        return Some("Deployment id is too long.".to_owned());
    }
    if value
        .chars()
        .any(|c| c == '"' || c == '\\' || c.is_control() || c.is_whitespace())
    {
        return Some("Deployment id contains characters that cannot be stored safely.".to_owned());
    }
    None
}
...
pub fn wizard(home: &Path, input: &mut dyn std::io::BufRead, secrets: &SecretInput) -> Result<WizardOutcome> {
    ...
    let console_setup =
        if ask_yes_no("\nConnect to the hosted console? [y/N]: ", input, false).unwrap_or(false) {
            let console_url = ask_validated_default(
                "  Console URL [https://api.agentvisorai.me]: ",
                "https://api.agentvisorai.me",
                input,
                |value| upstream_url_error(value).map(|reason| format!("That URL {reason}.")),
            )?;
            let deployment = ask_validated("  Deployment id: ", input, console_deployment_error)?;
            let mut token: Option<zeroize::Zeroizing<String>> = None;
            for _ in 0..5 {
                let pasted = ask_secret("  Paste your console ingest token", input, secrets)?;
                let trimmed = pasted.trim();
                if trimmed.is_empty() {
                    println!("The token looked empty — please paste it again.");
                    continue;
                }
                if trimmed.chars().any(char::is_control) {
                    println!("That token contains characters that don't belong — please try again.");
                    continue;
                }
                if !trimmed.is_ascii() {
                    println!("That token contains non-ASCII characters — please paste just the token.");
                    continue;
                }
                if trimmed.len() > 8192 {
                    println!("That looks far too long to be an ingest token — please paste just the token.");
                    continue;
                }
                token = Some(zeroize::Zeroizing::new(trimmed.to_owned()));
                break;
            }
            let Some(token) = token else {
                anyhow::bail!("no console ingest token provided");
            };
            let token_path = root.join("keys").join("console-ingest.token");
            write_private_key_file(&token_path, token.as_str())?;
            let bridge_dir = data_root.join("data").join("bridge");
            let plan = ConsoleSetupPlan {
                url: console_url,
                deployment,
                token_file: path_for_config("console ingest token file", &token_path)?,
                bridge_dir: path_for_config("console bridge directory", &bridge_dir)?,
            };
            Some((
                console_config_path_from(home),
                render_console_config(&plan),
                token_path,
            ))
        } else {
            None
    ...
    av_harness::HarnessConfig::from_toml(&rendered)
        .map_err(|error| anyhow::anyhow!("generated config failed validation (bug): {error}"))?;
    if let Some((_, rendered_console, _)) = &console_setup {
        let value: toml::Value = toml::from_str(rendered_console)
            .map_err(|error| anyhow::anyhow!("generated console config failed validation (bug): {error}"))?;
        if value.get("console").and_then(toml::Value::as_table).is_none() {
            anyhow::bail!("generated console config did not include console settings");
        }
    }
    ...
    let mut console_config_path = None;
    if let Some((path, rendered_console, _token_path)) = &console_setup {
        av_core::fsutil::write_atomic(path, rendered_console.as_bytes())
        ...
        console_config_path = Some(path.clone());
    }
    ...
    if let Some((path, _, token_path)) = &console_setup {
        println!("  Console: {}", path.display());
        println!(
            "  Console token: {} (private to your user account)",
            token_path.display()
        );
        println!("  Console sync: run `avctl console-sync --watch`.");
    }
...
mod tests {
    ...
    #[test]
    fn wizard_console_setup_writes_separate_private_config() {
        let home = tempfile::tempdir().unwrap();
        let outcome = drive_wizard(home.path(), "11\ny\n\ndep_123\nconsole-token\nn\n").unwrap();
        assert!(!outcome.start_now);
        let console_path = outcome.console_config_path.expect("console config path");
        assert_eq!(console_path, home.path().join(".agentvisor/console.toml"));

        let harness_text = std::fs::read_to_string(&outcome.config_path).unwrap();
        assert!(
            !harness_text.contains("[console]"),
            "daemon config must not grow an unknown console table"
        );
        let harness_config = av_harness::HarnessConfig::from_toml(&harness_text).unwrap();

        let console_text = std::fs::read_to_string(&console_path).unwrap();
        let console: toml::Value = toml::from_str(&console_text).unwrap();
        let table = console.get("console").and_then(toml::Value::as_table).unwrap();
        assert_eq!(
            table.get("url").and_then(toml::Value::as_str),
            Some("https://api.agentvisorai.me")
        );
        assert_eq!(
            table.get("deployment").and_then(toml::Value::as_str),
            Some("dep_123")
        );
        assert_eq!(
            table.get("bridge_dir").and_then(toml::Value::as_str),
            Some(harness_config.bridge_data_dir.as_str())
        );
        let token_path = PathBuf::from(table.get("token_file").and_then(toml::Value::as_str).unwrap());
        assert_eq!(std::fs::read_to_string(&token_path).unwrap(), "console-token");
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt as _;
            assert_eq!(
            ...
            assert_eq!(
                std::fs::metadata(&console_path).unwrap().permissions().mode() & 0o777,
                0o600
            );
        }
    }
